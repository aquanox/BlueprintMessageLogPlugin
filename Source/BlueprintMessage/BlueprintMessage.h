...
public:
	 ...
	 *
	 * @param WorldContextObject world context (if available)
	 * @param Category message log category for this message
	 * @param Severity severity of this message
	 * @param Message initial message token (optional)
	 * @returns message instance
	 */
	UFUNCTION(BlueprintCallable, Category="Utilities|BlueprintMessage", meta=(WorldContext="WorldContextObject", CallableWithoutWorldContext, Severity="EBlueprintMessageSeverity::Info", Keywords="create,blueprint,message"))
	static UBlueprintMessage* CreateBlueprintMessage(const UObject* WorldContextObject, FName Category, EBlueprintMessageSeverity Severity, FText Message = INVTEXT(""));
	...
	// UFUNCTION(BlueprintCallable)
	void AddDynamicTextToken(FGetMessageDynamicText Value, FName Slot = TEXT(""));

	/* Add token with clickable URL */
	UFUNCTION(BlueprintCallable)
	void AddURLToken(FString Value, FText DisplayText = INVTEXT(""), FName Slot = TEXT(""));
