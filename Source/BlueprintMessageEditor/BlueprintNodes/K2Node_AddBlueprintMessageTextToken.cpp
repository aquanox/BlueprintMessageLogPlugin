...
namespace
{
	const FName PN_ABMTT_Message(TEXT("Message"));
	const FName PN_ABMTT_Slot(TEXT("Slot"));
	const FName PN_ABMTT_Format(TEXT("Format"));
}
...
void UK2Node_AddBlueprintMessageTextToken::AllocateDefaultPins()
{
	...
	CachedExecPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Execute);
	CachedThenPin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Then);

	auto CachedSelfPin = CreateSelfPin(GetTargetFunction());
	CachedSelfPin->PinFriendlyName = LOCTEXT("Target", "Target");
	CachedInputPins.Add(UEdGraphSchema_K2::PN_Self, CachedSelfPin);

	CachedMessagePin = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, UBlueprintMessage::StaticClass(), PN_ABMTT_Message);

	auto CachedSlotPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Name, PN_ABMTT_Slot);
	...
	CachedInputPins.Add(PN_ABMTT_Slot, CachedSlotPin);

	auto CachedFormatPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Text, PN_ABMTT_Format);
	...
	CachedInputPins.Add(PN_ABMTT_Format, CachedFormatPin);
...
void UK2Node_AddBlueprintMessageTextToken::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	*/
	...
	CallAddTokenNode->SetFromFunction(GetTargetFunction());
	...
	CompilerContext.MovePinLinksToIntermediate(*FindPinChecked(PN_ABMTT_Message, EGPD_Output), *CallAddTokenNode->GetReturnValuePin());
...
{
	return FindInputPin(PN_ABMTT_Slot);
}

UEdGraphPin* UK2Node_AddBlueprintMessageTextToken::GetFormatPin() const
{
	return FindInputPin(PN_ABMTT_Format);
}
