...
namespace
{
	const FName PN_ABMT_Chain(TEXT("Message"));
	const FName PN_ABMT_Slot(TEXT("Slot"));
}
...
void UK2Node_AddBlueprintMessageToken::AllocateDefaultPins()
{
	...
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Object, NAME_None, UBlueprintMessage::StaticClass(), PN_ABMT_Chain);

	// Create other param pins
	UEdGraphPin* SlotPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Name, PN_ABMT_Slot);
...
{
	return FactoryReference.ResolveMember<UFunction>(GetBlueprintClassFromNode());
}
...
#else
	FText NodeText = FText::FromString(ObjectTools::GetDefaultTooltipForFunction(GetTargetFunction()));
	FText FactoryText = FText::FromString(ObjectTools::GetDefaultTooltipForFunction(GetFactoryFunction()));
#endif
...
void UK2Node_AddBlueprintMessageToken::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	// set slot
	bIsErrorFree &= MovePinLinksToIntermediate(FindPinChecked(PN_ABMT_Slot, EGPD_Input),
											   AddTokenNode->FindPinChecked(PN_ABMT_Slot, EGPD_Input));
	...
	// move chain to knot
	bIsErrorFree &= MovePinLinksToIntermediate(FindPinChecked(PN_ABMT_Chain, EGPD_Output), KnotNode->GetOutputPin());
